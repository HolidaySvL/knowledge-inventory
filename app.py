...
def build_fallback_corrected_draft(
    ...
    lines.extend([
        ...
        "",
        "---",
        "Generated by AI Knowledge Intake Agent — Guided Pre-correction.",
    ])
...
def safe_count_bool(series) -> int:
    try:
...
def top_non_unknown_value(df: pd.DataFrame, column: str) -> str:
    if df is None or df.empty or column not in df.columns:
        return ""
    s = df[column].fillna("Unknown").astype(str).str.strip()
...
def top_issue_summary(issue_df: pd.DataFrame) -> tuple[str, int]:
    if issue_df is None or issue_df.empty or "issue_type" not in issue_df.columns:
        return "", 0
    vc = issue_df["issue_type"].value_counts()
    return str(vc.index[0]), int(vc.iloc[0])


def recommend_agent_plan(df: pd.DataFrame) -> Dict[str, Any]:
    """Create a lightweight agent plan from current inventory state.

    This is intentionally deterministic in the Streamlit demo. In enterprise mode,
    the same state object can be sent to an LLM planner or API agent.
    """
    if df is None or df.empty:
        return {
            "current_state": "No inventory available",
            "recommended_scope": "Batch-level",
            "recommended_action": "Run initial scan first",
            "reason": "The agent needs an inventory before it can plan the next step.",
            "expected_impact": "No action available yet.",
            "workflow_steps": ["Upload documents", "Run Knowledge Inventory Analysis"],
            "filters": {},
            "target_documents": [],
            "blockers": [],
        }

    issue_df = build_issue_registry(df)
    total_docs = len(df)
    high_count = int((df.get("risk_level", pd.Series(dtype=str)) == "High").sum()) if "risk_level" in df.columns else 0
    medium_count = int((df.get("risk_level", pd.Series(dtype=str)) == "Medium").sum()) if "risk_level" in df.columns else 0
    review_count = safe_count_bool(df["needs_human_review"]) if "needs_human_review" in df.columns else 0
    issue_count = len(issue_df)
    top_issue, top_issue_count = top_issue_summary(issue_df)

    blockers = []
    if issue_df is not None and not issue_df.empty and "extraction_incomplete" in issue_df["issue_type"].tolist():
        blockers.append("Some documents have extraction limitations. These should not be published without review.")
    if issue_df is not None and not issue_df.empty and "conflict_detected" in issue_df["issue_type"].tolist():
        blockers.append("Potential document conflicts were detected and should be clarified before ingestion.")

    # Find concentrated process / department risk cluster.
    top_department = ""
    top_process = ""
    segment_doc_count = 0
    if not issue_df.empty:
        group_cols = [c for c in ["department", "business_process"] if c in issue_df.columns]
        if group_cols:
            grouped = issue_df.copy()
            for c in group_cols:
                grouped[c] = grouped[c].fillna("Unknown").astype(str)
            g = grouped.groupby(group_cols)["document_name"].nunique().sort_values(ascending=False)
            if not g.empty:
                key = g.index[0]
                segment_doc_count = int(g.iloc[0])
                if isinstance(key, tuple):
                    if len(key) > 0: top_department = key[0]
                    if len(key) > 1: top_process = key[1]
                else:
                    top_department = str(key)

    # Recommend a route.
    if issue_count == 0 or (high_count == 0 and medium_count == 0 and review_count == 0):
        recommended_scope = "Export"
        recommended_action = "Export KB-ready package"
        reason = "The current inventory does not show high-impact issues requiring pre-correction."
        expected = "You can download the inventory, risk report, and KB-ready outputs now."
        steps = [
            "Confirm dashboard summary",
            "Download report bundle",
            "Optionally run document-level review for critical source-of-truth files",
        ]
    elif top_issue_count >= max(3, int(total_docs * 0.25)):
        recommended_scope = "Batch-level"
        recommended_action = "Run batch-level pre-correction first"
        reason = f"The same issue pattern appears repeatedly across the batch: {issue_label(top_issue)} ({top_issue_count} issue records)."
        expected = "One clarification answer may correct multiple documents and reduce repeated manual review."
        steps = [
            "Run batch-level guided questions",
            "Apply only confirmed answers",
            "Review the updated dashboard",
            "Move remaining high-risk documents into segment or document-level correction",
        ]
    elif segment_doc_count >= 2 and (top_department or top_process):
        recommended_scope = "Segment-level"
        label_parts = [p for p in [top_department, top_process] if p and p.lower() != "unknown"]
        seg_label = " / ".join(label_parts) if label_parts else "the largest issue segment"
        recommended_action = f"Refine the {seg_label} segment"
        reason = f"Issues are concentrated in a specific department/process segment ({segment_doc_count} affected document(s))."
        expected = "Segment-level correction can apply more precise business rules without over-correcting the entire batch."
        steps = [
            "Filter the dashboard to the affected segment",
            "Run segment-level guided questions",
            "Apply answers only to the selected scope unless explicitly confirmed broader",
            "Generate corrected documents and ledger",
        ]
    else:
        recommended_scope = "Document-level"
        target_docs = []
        if "risk_level" in df.columns:
            risk_order = df["risk_level"].map({"High": 0, "Medium": 1, "Low": 2}).fillna(3)
            target_docs = df.assign(_risk_order=risk_order).sort_values("_risk_order")["document_name"].dropna().astype(str).head(1).tolist()
        recommended_action = "Run document-level correction for the highest-risk file"
        reason = "Issues do not form a strong shared batch or segment pattern, so precise review is safer."
        expected = "This prevents one document's clarification from accidentally changing unrelated files."
        steps = [
            "Open the highest-risk document",
            "Run document-level clarification",
            "Apply corrections to this document only",
            "Generate corrected draft and ledger",
        ]
        return {
            "current_state": f"Initial scan completed: {total_docs} document(s), {high_count} high-risk, {review_count} needing review.",
            ...
            "top_issue": issue_label(top_issue) if top_issue else "None",
        }

    return {
        "current_state": f"Initial scan completed: {total_docs} document(s), {high_count} high-risk, {medium_count} medium-risk, {review_count} needing review.",
        "recommended_scope": recommended_scope,
        "recommended_action": recommended_action,
        "reason": reason,
        "expected_impact": expected,
        "workflow_steps": steps,
        "filters": {
            "department": top_department if recommended_scope == "Segment-level" else "",
            "business_process": top_process if recommended_scope == "Segment-level" else "",
            "risk_level": ["High", "Medium"] if recommended_scope == "Segment-level" else [],
        },
        "target_documents": [],
        "blockers": blockers,
        "issue_count": issue_count,
...
def scope_df_from_agent_plan(df: pd.DataFrame, plan: Dict[str, Any]) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    scope = plan.get("recommended_scope", "Batch-level")
    scope_df = df.copy()

    if scope == "Segment-level":
        filters = plan.get("filters", {}) or {}
        dept = filters.get("department", "")
        process = filters.get("business_process", "")
        risks = filters.get("risk_level", [])
        if dept and dept.lower() != "unknown" and "department" in scope_df.columns:
            scope_df = scope_df[scope_df["department"].astype(str) == dept]
        if process and process.lower() != "unknown" and "business_process" in scope_df.columns:
            scope_df = scope_df[scope_df["business_process"].astype(str) == process]
        if risks and "risk_level" in scope_df.columns:
            scope_df = scope_df[scope_df["risk_level"].isin(risks)]
        return scope_df

    if scope == "Document-level":
        targets = plan.get("target_documents", []) or []
        if targets and "document_name" in scope_df.columns:
            return scope_df[scope_df["document_name"].isin(targets[:1])]
        return scope_df.head(1)

    if scope == "Selected-documents":
        targets = plan.get("target_documents", []) or []
        if targets and "document_name" in scope_df.columns:
            return scope_df[scope_df["document_name"].isin(targets)]
        return scope_df.head(0)

    return scope_df


def run_agent_recommended_step(df: pd.DataFrame, ai_config: Dict[str, Any], plan: Dict[str, Any]):
    scope = plan.get("recommended_scope", "Batch-level")
    if scope == "Export":
        st.info("Agent recommendation is to export current outputs. No pre-correction is required for the current inventory state.")
        return

    scope_df = scope_df_from_agent_plan(df, plan)
    if scope_df is None or scope_df.empty:
        st.warning("The recommended scope has no matching documents. Choose another scope manually.")
        st.session_state.show_precorrection = True
        return

    st.session_state.agent_preselected_scope = scope
    filters = plan.get("filters", {}) or {}
    st.session_state.agent_preselected_department = filters.get("department", "")
    st.session_state.agent_preselected_process = filters.get("business_process", "")
    st.session_state.agent_preselected_risks = filters.get("risk_level", [])
    targets = plan.get("target_documents", []) or []
    st.session_state.agent_preselected_document = targets[0] if targets else ""
    st.session_state.show_precorrection = True

    qdf = generate_scope_questions(df, scope, scope_df)
    st.session_state.correction_questions = qdf
    st.session_state.correction_scope_docs = scope_df["document_name"].dropna().tolist() if "document_name" in scope_df.columns else []
    st.session_state.correction_scope_type = scope
    st.session_state.agent_last_action = f"Prepared {scope} guided questions from Agent Planner."


def render_agent_control_room(df: pd.DataFrame, ai_config: Dict[str, Any]):
    ...
    if st.session_state.get("agent_plan") is None:
        st.session_state.agent_plan = recommend_agent_plan(df)

    plan = st.session_state.agent_plan or recommend_agent_plan(df)

    c1, c2, c3 = st.columns([1.1, 1.1, 1])
    with c1:
        st.metric("Recommended scope", plan.get("recommended_scope", ""))
    with c2:
        st.metric("Detected issues", plan.get("issue_count", 0))
    with c3:
        st.metric("Top issue", plan.get("top_issue", "None"))

    st.markdown(f"**Current state**  \n{plan.get('current_state', '')}")
    st.markdown(f"**Recommended next action**  \n{plan.get('recommended_action', '')}")
    st.markdown(f"**Why this path**  \n{plan.get('reason', '')}")
    ...
    if blockers:
        st.warning("Agent guardrails: " + " ".join(blockers))
    ...
    with b:
        if st.button("Choose Another Scope", use_container_width=True):
            st.session_state.show_precorrection = True
            st.session_state.agent_preselected_scope = "Batch-level"
            st.session_state.correction_questions = None
    with c:
        if st.button("Skip Agent & Export", use_container_width=True):
            st.session_state.show_precorrection = False
            st.info("You can download the current reports from the Download Reports section below.")

    if st.session_state.get("agent_last_action"):
        st.success(st.session_state.agent_last_action)
...
def render_precorrection_workbench(base_df: pd.DataFrame, ai_config: Dict[str, Any]):
    ...
    with c1:
        scope_options_main = ["Batch-level", "Segment-level", "Selected-documents", "Document-level"]
        preselected_scope = st.session_state.get("agent_preselected_scope", "Batch-level")
        preselected_index = scope_options_main.index(preselected_scope) if preselected_scope in scope_options_main else 0
    ...
    with c2:
        if scope_choice == "Segment-level":
            ...
            agent_dept = st.session_state.get("agent_preselected_department", "")
            dept_default = [agent_dept] if agent_dept in dept_options else (dept_options[:1] if dept_options else [])
            selected_depts = st.multiselect("Department", dept_options, default=dept_default)
            if selected_depts:
    ...
    with c3:
        if scope_choice == "Segment-level":
            ...
            agent_process = st.session_state.get("agent_preselected_process", "")
            process_default = [agent_process] if agent_process in process_options else []
            selected_processes = st.multiselect("Process", process_options, default=process_default)
    ...
    with c4:
        if scope_choice == "Segment-level":
            ...
            agent_risks = st.session_state.get("agent_preselected_risks", [])
            risk_default = [r for r in agent_risks if r in risk_options] or risk_options
            selected_risks = st.multiselect("Risk", risk_options, default=risk_default)
            if selected_risks:
    ...
    if scope_choice == "Selected-documents":
        ...
        doc_options = base_df["document_name"].dropna().tolist()
        agent_doc = st.session_state.get("agent_preselected_document", "")
        doc_index = doc_options.index(agent_doc) if agent_doc in doc_options else 0
        one_doc = st.selectbox("Select one document", doc_options, index=doc_index) if doc_options else None
...
st.set_page_config(page_title="AI Knowledge Intake Agent", page_icon="🧭", layout="wide")
...
.agent-card {
  ...
  background: linear-gradient(135deg, rgba(15, 23, 42, 0.035), rgba(14, 165, 233, 0.08), rgba(255,255,255,0.95));
...
st.markdown("""
<div class="hero">
  <div class="hero-title">🧭 AI Knowledge Intake Agent</div>
...
for key, default in {
    ...
    "agent_plan": None,
    "agent_preselected_scope": "Batch-level",
    "agent_preselected_department": "",
    "agent_preselected_process": "",
    "agent_preselected_risks": [],
    "agent_preselected_document": "",
    "agent_last_action": "",
}.items():
...
def run_analysis(extracted_docs):
    ...
    st.session_state.agent_preselected_scope = "Batch-level"
    st.session_state.agent_preselected_department = ""
    st.session_state.agent_preselected_process = ""
    st.session_state.agent_preselected_risks = []
    st.session_state.agent_preselected_document = ""
    st.session_state.agent_last_action = ""
...
if st.session_state.inventory is not None:
    ...
    <div class="precorrect-cta">
      <div class="precorrect-title">Need to correct the diagnosis manually?</div>
      <div class="precorrect-copy">
        You can follow the agent recommendation above, or open guided pre-correction yourself to choose batch, segment, selected-document, or document-level correction.
      </div>
    </div>
    """, unsafe_allow_html=True)
